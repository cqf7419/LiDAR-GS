...
class DynamicScene:
    ...
    def __init__(self, args : ModelParams, model_id : int, waymo_dynamic_model, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], ply_path=None):
        """b
        :param path: Path to colmap scene main folder.
        """
        self.model_path = args.model_path
        ...
        scene_info = sceneLoadTypeCallbacks["waymo"](model_id, waymo_dynamic_model, args.source_path, args.white_background, args.eval)
    ...
    def save(self, iteration, model_id):
        point_cloud_path = os.path.join(self.model_path, str(model_id), "point_cloud/iteration_{}".format(iteration))
        self.gaussians.save_ply(os.path.join(point_cloud_path, "point_cloud.ply"))
        self.gaussians.save_mlp_checkpoints(point_cloud_path, mode = 'unite')
