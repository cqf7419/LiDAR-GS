...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    print("[debug] scene dataset data_label:",dataset.data_label)
    ...
    ema_dp_for_log = 0.0
    ema_dgx_for_log = 0.0
    progress_bar = tqdm(range(first_iter, opt.iterations),leave=False, dynamic_ncols=True, desc="Training progress")
    first_iter += 1
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        image, depth, viewspace_point_tensor, visibility_filter, offset_selection_mask, radii, scaling, opacity = render_pkg["render"], render_pkg["depth"],render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["selection_mask"], render_pkg["radii"], render_pkg["scaling"], render_pkg["neural_opacity"]
        ...
        if gaussians.color_channel>1:
            ...
            raydrop_loss = 10 * mse_loss(render_raydrop,ray_drop)   # lambada = 10 waymo / 1 kitti
        else:
            render_intensity = render_intensity*ray_drop 
            depth = depth*ray_drop
        ...
        # ---------------------- scale reg ----------------------------#
        scaling_reg = 0.01 * scaling.prod(dim=1).mean() # 0.05 or 0.01
        ...
        # ------------------------ grad loss -------------------------#
        # pred_grad_y = torch.abs(depth[:, :, :-1] - depth[:, :, 1:])
        ...
        # mask_dy = ray_drop[:, :-1, :] * grad_mask_y
        grad_loss = l1_loss(pred_grad_x * mask_dx, gt_grad_x * mask_dx) #+ l1_loss(pred_grad_x * grad_mask_x_edge, gt_grad_x * grad_mask_x_edge)) 

        intensity_loss = ((1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss)
 
        loss = depth_loss + intensity_loss  + raydrop_loss  + scaling_reg + grad_loss   # + loss_opa
        loss.backward()
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, opt, dataset_name, iteration, Ll1, depth_loss, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), wandb, logger)
            if (iteration in saving_iterations):
                print("max r: ", torch.max(radii))
                logger.info("\n[ITER {}] Saving Gaussians".format(iteration))
                scene.save(iteration)
                
            
            # densification
            if iteration < opt.update_until and iteration > opt.start_stat and gaussians.get_anchor.shape[0] < opt.densify_until_num_points:
                # add statis
...
def training_report(tb_writer, opt, dataset_name, iteration, Ll1, depth_loss, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, wandb=None, logger=None):
    if tb_writer:
