...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        args = (
            ...
            raster_settings.beam_inclinations,
            ...
            raster_settings.lidar_far,
            raster_settings.lidar_near,
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, depth, radii, pixels, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)
    ...
    @staticmethod
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_pix):
        ...
        if raster_settings.debug:
            ...
            try:
             ...
             grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_sh, grad_scales, grad_rotations, depth = _C.rasterize_gaussians_backward(*args)
        # #"Calculate the scaling factor" from  pixel-gs
        # scaling_factor = torch.minimum(torch.ones_like(depth), (depth / raster_settings.depth_threshold) ** 2)

        # #  new version
        # scaling_factor = torch.ones_like(depth)
        # scaling_factor[depth <= raster_settings.depth_threshold] = (depth[depth <= raster_settings.depth_threshold] / raster_settings.depth_threshold)**2
        # th = 150 #raster_settings.depth_threshold/0.37
        # scaling_factor[depth > th] = torch.exp((depth[depth > th] - th)*(-0.5))
        ...
        # if True: # pixel-gs
        #     grad_means2D_final = scaled_grad_means2D
        # else :
        grad_means2D_final = grad_means2D
        grads = (
            grad_means3D,
            grad_means2D_final,
            grad_sh,
