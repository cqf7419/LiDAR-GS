...
__device__ const float far_n = 80.0;
	...
	float3& p_view)
{
	...
	float dis = sqrtf(p_view.x*p_view.x + p_view.z*p_view.z);
	
	float fovx = atan2f(p_view.x, p_view.z);
	if (fovx < -1.658 || fovx>1.658 ) // || p_view.z<0
	{
		if (prefiltered)
		{
