	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, out_color, out_others, radii, pixels, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dtransMat_2dtemp =  torch::zeros({P, 3}, means3D.options());
  torch::Tensor dL_dsh = torch::zeros({P, M, 3}, means3D.options());
  torch::Tensor dL_dscales = torch::zeros({P, 2}, means3D.options());
  torch::Tensor dL_drotations = torch::zeros({P, 4}, means3D.options());
  torch::Tensor depth = torch::zeros({P, 1}, means3D.options());
  ...
  {  
	  ...
	  dL_dtransMat_2dtemp.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations, depth);
}
