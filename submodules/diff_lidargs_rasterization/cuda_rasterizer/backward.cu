	...
	float* __restrict__ dL_ddepths)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float3 unit_sphere_xyz = collected_sphere_means3D[j]; //单位球上gs的分布 投影矩阵的中心 （减了雷达pos又除了深度到单位球）
			...
			const float2 d = {_d_u1/u1_u1, _d_u2/u2_u2}; // TODO 这个距离是相对于基向量的 是否要乘基向量转到世界坐标系待定
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				if(ch==0) // raydrop通道 不希望其对透明度有影响
					dL_dalpha += (c - accum_rec[ch]) * dL_dchannel;// 透明度只有一维 所以三个通道的grad加到一起
				// if(ch==1)
				// 	dL_dalpha += (c - accum_rec[ch]) * dL_dchannel / 10;
			...
			const float dep = collected_depths[j]*lambda_D;
			accum_red = last_alpha * last_depth + (1.f - last_alpha) * accum_red;
			last_depth = dep;
			dL_dalpha += (dep-accum_red) * dL_dout_depth;
			atomicAdd(&(dL_ddepths[global_id]), dchannel_dcolor * dL_dout_depth *lambda_D);
            ...
            // atomicAdd(&dL_dmean2D[global_id].z, fabs(dL_dG * dG_ddelx / collected_depths[j]*0.5*float(W) ));//
            // atomicAdd(&dL_dmean2D[global_id].w, fabs(dL_dG * dG_ddely / collected_depths[j]*0.5*float(H) ));//
			
			const float dlambdaD_dsx = -1.0 * uint_sphere_pixf.x / (cos_phi*cos_phi);
			const float dlambdaD_dsy = -1.0 * uint_sphere_pixf.y / (cos_phi*cos_phi);
			const float dlambdaD_dsz = -1.0 * uint_sphere_pixf.z / (cos_phi*cos_phi);
			const float dL_dlambdaD = dL_dout_depth * dchannel_dcolor * collected_depths[j];
			// D = collected_depth[j] * lambda_D * alpha * T

			const float3 ddsx_ds = {
				1.0 - uint_sphere_pixf.x * dlambdaD_dsx,
				0 - uint_sphere_pixf.x * dlambdaD_dsy,
				0 - uint_sphere_pixf.x * dlambdaD_dsz,
			};
			const float3 ddsy_ds = {
				0 - uint_sphere_pixf.y * dlambdaD_dsx,
				1.0 - uint_sphere_pixf.y * dlambdaD_dsy,
				0 - uint_sphere_pixf.y * dlambdaD_dsz,
			};
			const float3 ddsz_ds = {
				0 - uint_sphere_pixf.z * dlambdaD_dsx,
				0 - uint_sphere_pixf.z * dlambdaD_dsy,
				1.0 - uint_sphere_pixf.z * dlambdaD_dsz,
			};
			const float ddx_ddsx = u1.x/u1_u1;
			const float ddx_ddsy = u1.y/u1_u1;
			const float ddx_ddsz = u1.z/u1_u1;
			const float ddy_ddsx = u2.x/u2_u2;
			const float ddy_ddsy = u2.y/u2_u2;
			const float ddy_ddsz = u2.z/u2_u2;
			const float dG_dsx = dG_ddelx * (ddx_ddsx * ddsx_ds.x + ddx_ddsy * ddsy_ds.x + ddx_ddsz * ddsz_ds.x) \
							+ dG_ddely * (ddy_ddsx * ddsx_ds.x + ddy_ddsy * ddsy_ds.x + ddy_ddsz * ddsz_ds.x);
			const float dG_dsy = dG_ddelx * (ddx_ddsx * ddsx_ds.y + ddx_ddsy * ddsy_ds.y + ddx_ddsz * ddsz_ds.z) \
							+ dG_ddely * (ddy_ddsx * ddsx_ds.y + ddy_ddsy * ddsy_ds.y + ddy_ddsz * ddsz_ds.y);
			const float dG_dsz = dG_ddelx * (ddx_ddsx * ddsx_ds.z + ddx_ddsy * ddsy_ds.z + ddx_ddsz * ddsz_ds.z) \
							+ dG_ddely * (ddy_ddsx * ddsx_ds.z + ddy_ddsy * ddsy_ds.z + ddy_ddsz * ddsz_ds.z);
			float dL_dsphere_means3Dx = dL_dlambdaD * dlambdaD_dsx + dL_dG * dG_dsx;
			float dL_dsphere_means3Dy = dL_dlambdaD * dlambdaD_dsy + dL_dG * dG_dsy;
			float dL_dsphere_means3Dz = dL_dlambdaD * dlambdaD_dsz + dL_dG * dG_dsz;
			...
			float x2y2 = 1;//sqrtf(unit_sphere_xyz.y*unit_sphere_xyz.y + unit_sphere_xyz.x*unit_sphere_xyz.x);
