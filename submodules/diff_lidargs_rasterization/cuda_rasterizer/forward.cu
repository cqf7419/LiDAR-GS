...
__device__ __constant__ float Ray_Divergence_Angle = 0.006; //GT      waymo 0.002  kitti 0.004-0.003
	...
	const int near)  // 
{
	...
	float3 p_view = transformPoint4x3(p_orig, viewmatrix); // 原点0，0，0 == lidar center
	...
	else
	{
		...
		cov3D = cov3Ds + idx * 6;  // 从四元素rot和scale求gs椭球的3d协方差矩阵
	}

	// Compute 2D rangeview-space covariance matrix
	
	// glm::mat3 Proj = proj_2basis(p_orig, *cam_pos);
	glm::mat3 Proj = _proj_2basis(p_view);
	float3 cov = computeCov2D_lidar(Proj, cov3D, viewmatrix);  
	// cov = 1/(dist*dist) * cov; // 规范化到单位空间
	cov.x = cov.x / (dist*dist);
	cov.y = cov.y / (dist*dist);
	cov.z = cov.z / (dist*dist);
	float det = (cov.x * cov.z - cov.y * cov.y); // 计算特征矩阵的模 |ad-bc|  0.001742486
	// if(idx==170725){
	// 	printf("det value: %f\n",det);
	// }
	if (det == 0.0f) return;
	float det_inv = 1.f / det; //573.892829526  
	float3 conic = { cov.z * det_inv, -cov.y * det_inv, cov.x * det_inv }; // 2d协方差矩阵的逆 （主对角线调换，副对角线加负号，然后除模） 高斯的方程e的阶次里面计算的协方差矩阵就是逆，对应上了
	// 韦达定理求二阶矩阵的特征值
	...
	float p_c = beta / (2 * pi / W); // 保留小数位 //round(beta / (2 * pi / W)); TODO 是否四舍五入 待定
	...
	float after = 0;
	float p_r = 0;
	// float fs_alpha = 5 * max(0.4, float((far-dist)/far));
	if (p_r_int>0){
		...
		if( alpha > (after + Ray_Divergence_Angle)) return; 
		// if( alpha > (before + Ray_Divergence_Angle*fs_alpha) && alpha < (after - Ray_Divergence_Angle*fs_alpha) ) return;
	}
	else{ // p_r_int==0
		...
		if( alpha < (before - Ray_Divergence_Angle) ) return;
		// if( alpha > (before + Ray_Divergence_Angle*fs_alpha) && alpha < (after - Ray_Divergence_Angle*fs_alpha) ) return;
	}
	p_r = H-p_r-1;

	// 这个半径用来在pix选择哪些gs在哪些block已进行渲染
	// float fs_y = 3.0 * max(dist*2/far, 1.0);
	float fs_y = 3.0;
	int my_radius_y = ceil(fs_y * my_radius / tan(abs(after - before))); // 对于y轴来说 雷达不是等分的距离，且间隔的度数也不同 对标cam的内参fx和fy
	int my_radius_x = ceil(3.f *my_radius / tan(2*pi/W)); //等分的 x轴 单位从m到pix  TODO sin还是tan待定
	float2 point_image = {p_c, p_r}; // {W,H}
	// if(idx==0) printf("r_x&r_y: %i %i\n pc&pr: %f %f\n",my_radius_x,my_radius_y,p_c,p_r);

	uint2 rect_min, rect_max;
	getRect_lidar(point_image, my_radius_x, my_radius_y, rect_min, rect_max, grid);  // 检测椭圆覆盖了哪些tile  
	...
	depths[idx] = dist;  // 点到相机距离即为深度 
	// printf("my_radius value: %f %f %f\n",idx,my_radius_x,my_radius_y);
	radii[idx] = max(my_radius_x, my_radius_y);  // 高斯半径
	radii_xy[2*idx+0] = my_radius_x;
	radii_xy[2*idx+1] = my_radius_y;
	points_xy_image[idx] = point_image;

	basis_u1[idx] = {Proj[0][0], Proj[0][1], Proj[0][2]}; // 这才是对的，难道索引也是列优先？？？
	basis_u2[idx] = {Proj[1][0], Proj[1][1], Proj[1][2]};
	// sphere_means3D[idx] = {(p_orig.x - v_cam_pos.x)/dist, (p_orig.y - v_cam_pos.y)/dist, (p_orig.z - v_cam_pos.z)/dist};
	sphere_means3D[idx] = {p_view.x/dist, p_view.y/dist, p_view.z/dist};


	tiles_touched[idx] = (rect_max.y - rect_min.y) * (rect_max.x - rect_min.x); // 计算第idx个高斯覆盖的block个数
}
	...
	const int near)
{
	...
	// float3 cov = computeCov2D_lidar((glm::vec3)p_orig, cam_pos, cov3D);  
	// glm::mat3 Proj = proj_2basis(p_orig, *cam_pos);
	glm::mat3 Proj = _proj_2basis(p_view);
	float3 cov = computeCov2D_lidar(Proj, cov3D, viewmatrix);   
	cov.x = cov.x / (dist*dist);
	cov.y = cov.y / (dist*dist);
	cov.z = cov.z / (dist*dist);//规范化到单位空间
	float det = (cov.x * cov.z - cov.y * cov.y); // 计算特征矩阵的模 |ad-bc|
	// if(idx==170725){
	// 	printf("det value: %f\n",det);
	// }
	if (det == 0.0f) return;
	...
	// 极坐标系 光栅化gs
	float beta = pi - atan2(p_view.y, p_view.x);
	float p_c = beta / (2 * pi / W); // 保留小数位 //round(beta / (2 * pi / W)); 

	float alpha = atan2(p_view.z, sqrt(max(1e-9,p_view.x*p_view.x + p_view.y*p_view.y))); // [-pi,pi] 弧度
	int p_r_int = find_closest_label(beam_inclinations, alpha, H); // 二分最近邻搜索
	float before = 0;
	float after = 0;
	float p_r  = 0;
	if (p_r_int>0){
	...
	float* __restrict__ out_occ)
{
	...
	auto block = cg::this_thread_block();  // 线程块block
	uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X; // 把x轴方向根据 BLOCK_X 大小分成 horizontal_blocks 个 tiles block
	uint2 pix_min = { block.group_index().x * BLOCK_X, block.group_index().y * BLOCK_Y }; // 根据现在的group id 得到需要计算的 pix 范围
	uint2 pix_max = { min(pix_min.x + BLOCK_X, W), min(pix_min.y + BLOCK_Y , H) };// 对应的 pix 范围 [min,max]
	uint2 pix = { pix_min.x + block.thread_index().x, pix_min.y + block.thread_index().y };  // 根据现在的thread id 计算当前线程在处理的像素坐标
	uint32_t pix_id = W * pix.y + pix.x;    // 像素的id 用于最后索引位置更新渲染结果
	float2 pixf = { (float)pix.x, (float)pix.y };

	// Check if this thread is associated with a valid pixel or outside.
	bool inside = pix.x < W&& pix.y < H;   
	// Done threads can help with fetching, but don't rasterize
	bool done = !inside;  // 成像范围

	// Load start/end range of IDs to process in bit sorted list.  // 以下：一个block对应一个tile  一个线程对应一个像素
	uint2 range = ranges[block.group_index().y * horizontal_blocks + block.group_index().x]; 
	// 【功能】：当前tile在points_key_list中的范围  【实现】：通过block id去索引该像素线程涉及到计算多少个gs  【注释】ranges每行有horizontal_blocks个block，在纵轴上第block.group_index().y，横轴上第block.group_index().x个则对应就是第block.group_index().y * horizontal_blocks + block.group_index().x个block
	const int rounds = ((range.y - range.x + BLOCK_SIZE - 1) / BLOCK_SIZE); //// BLOCK_SIZE = 16*16=256  当前线程任务会被分成多少个patch去计算  （？因为每个tile（block）是16*16 因此分母就是BLOCK_SIZE。 int类型 整数 1，2，...）
	int toDo = range.y - range.x; // 总共当前线程块计算的gs数量

	// Allocate storage for batches of collectively fetched data.
	__shared__ int collected_id[BLOCK_SIZE];  // __shared__: 同一block中的线程共享的内存
	...
	float T = 1.0f;  // T = transmittance，透光率  （透明度的累乘）
	uint32_t contributor = 0;  // 多少个高斯对该像素有贡献
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) 
	{
		...
		int num_done = __syncthreads_count(done); // 具有线程计数的功能， 同时用于同步block中的所有线程
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// float2 xy = collected_xy[j]; // gs中点在图像中的位置

			float2 d;
			float3 unit_sphere_xyz = collected_sphere_means3D[j]; //单位球上gs的分布 投影矩阵的中心 （减了雷达pos又除了深度）
			float3 u1 = collected_basis_u1[j]; // 投影平面的基向量
			float3 u2 = collected_basis_u2[j];
			// if( (H-1-pix.y)<0 || (H-1-pix.y)>=64) printf("error key in beam_inclinations!!");
			...
			float cos_phi = uint_sphere_pixf.x * unit_sphere_xyz.x + uint_sphere_pixf.y * unit_sphere_xyz.y + uint_sphere_pixf.z * unit_sphere_xyz.z; // 分母两个都是单位向量所以为1
			float lambda_D =  1.0 / cos_phi;
			if(lambda_D<=0) continue; // 不交
			float3 uint_sphere_d = {unit_sphere_xyz.x - uint_sphere_pixf.x*lambda_D, unit_sphere_xyz.y - uint_sphere_pixf.y*lambda_D, unit_sphere_xyz.z - uint_sphere_pixf.z*lambda_D}; // 规范空间下的相对位置
			// d = a*u1 + b*u2 //uint_sphere_d在切平面上的投影可以用两个基向量表示 a =collected_depth (_d*u1)/(u1*u1) b = (_d*u2)/(u2*u2)
			...
			d = {_d_u1/u1_u1, _d_u2/u2_u2}; // 投影平面上的偏差 相对于u1和u2坐标轴 还是得使用单位向量作为投影基 保证尺度始终统一
			// if(H==32 && W==1200) printf("H=32 & W=1200 's d: %f %f\n",d.x,d.y);


			// float2 d = { xy.x - pixf.x, 1*(xy.y - pixf.y) }; // block中各像素到gs中心的偏差
			float4 con_o = collected_conic_opacity[j];
			float power = -0.5f * (con_o.x * d.x * d.x + con_o.z * d.y * d.y) - con_o.y * d.x * d.y; // gs论文公式4 （概率分布函数） 矩阵相乘的结果
			if (power > 0.0f) // 该系数必须是小于零
				continue;
			...
			// -0.5 * (100 * max(r-0.5,0))
			// power = min(power+5 , 0.0);
			float alpha = min(0.99f, con_o.w * exp(power));
			if (alpha < 1.0f / 255.0f) // 不透明度很小就不渲染
				continue;
			float test_T = T * (1 - alpha);  // 透光率累乘 
			if (test_T < 0.0001f) // 透光率很小就不渲染
			{
				done = true;
				continue;
			}
			...
			// if(depth_cos_theta == 0) printf("[Forward Error]:depth_cos_theta shouldn't be zero!! A: %f,%f,%f  B: %f,%f,%f",unit_sphere_xyz.x,unit_sphere_xyz.y,unit_sphere_xyz.z,uint_sphere_pixf.x,uint_sphere_pixf.y,uint_sphere_pixf.z);
            // D += (collected_depth[j]/depth_cos_theta) * alpha * T; 
			D += collected_depth[j] * lambda_D * alpha * T; // lidar深度
	...
	if (inside)  // 该像素是视野内的像素 则更新out的结果
	{
		final_T[pix_id] = T;
